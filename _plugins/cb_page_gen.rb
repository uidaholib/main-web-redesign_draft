  ...
  class ItemPageGenerator < Jekyll::Generator
    ...
    def generate(site)
      ...
      configure_gen.each do |data_config|
        ...
        template_test = records.map { |x| x[display_template] ? "item/" + x[display_template].strip : template }.uniq
        #puts "#{template_test}"
        ...
        records.each_with_index do |record, index|
          ...
          if record[display_template] 
            item_layout = "item/" + record[display_template].strip
            if all_layouts.include? item_layout 
              record['layout'] = item_layout
            else
